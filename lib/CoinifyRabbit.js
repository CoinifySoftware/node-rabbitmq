...
class CoinifyRabbit {
  ...
  constructor(options = {}) {
    ...
    this._config = _.defaultsDeep({}, options, DEFAULT_CONFIG);
  ...
  async _handleConsumeMessage(message, messageType, options, consumeFn) {
    ...
    this._logger.info({ [messageType]: msgObj }, `${messageType} ready to be consumed`);
  ...
  async _handleConsumeRejection(message, messageType, messageObject, consumeError, options) {
    ...
    const onError = _.has(options, 'onError') ?
      ...
      async () => {
        ...
        this._logger.error({ err: consumeError, [messageType]: messageObject }, `Error consuming ${messageType}`);
      };
    ...
    if (shouldRetry) {
      ...
      this._logger.error(logContext, `${messageType} ${messageName} sent to dead-letter exchange without retry`);
    }
  }
