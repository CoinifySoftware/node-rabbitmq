...
class CoinifyRabbit {
  ...
  async registerFailedMessageConsumer(consumeFn, options = {}) {
    const channel = await this._getChannel();
    const consumeMessageOptions = _.defaultsDeep({queueName}, options);
    ...
    await channel.prefetch(prefetch, false);
    const {consumerTag} = await channel.consume(q.queue,
      async (message) => this._handleFailedMessage(message, consumeMessageOptions, consumeFn),
      {consumerTag: options.consumerTag}
    );

    this._registeredConsumers.push({type: 'failed', key: 'janitor', consumerTag, consumeFn, options});

    return consumerTag;
  }

  async _handleFailedMessage(message, options, consumeFn) {
    // message === null if consumer is cancelled: http://www.rabbitmq.com/consumer-ca]ncel.html
    if (message === null) {
      if (!options.onCancel) {
        return null;
      }
    ...
    this._logger.debug({failed: msgObj}, 'failed message ready to be consumed');
    ...
    try {
      ...
      this._logger.info({failed: msgObj, consumeResult: consumeResultTruncated, consumeTimeMillis}, 'message consumed');
    } catch (err) {
      consumeError = err;
    }
